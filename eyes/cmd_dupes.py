...
Wywolywane przez `python -m eyes dupes` (eyes/cli.py) i przez alias `scripts/duplikaty.py`.

Uzycie (venv):
  python -m eyes dupes --project "Canarian Tweety EP03" [--verify]
Czyta {cache_root}/{P}/_log.jsonl (wpisy status=duplikat z pomiaru: plik pominiety + duplikat_of).
...
Pisze: {color_dir}/kasacja_duplikaty.json i .md (lista dla Kuby).
Kasacja to decyzja czlowieka, poza tym skryptem.
"""
...
def parse_args(argv=None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Lista duplikatow z logu przemialu (sonny-eyes)")
...
def main(argv=None) -> int:
    ...
    md = [f"# Kasacja, krok 1: identyczne kopie ({a.project})", "",
          ...
          "Zasada: kopia to plik do usunięcia, oryginał zostaje. Lista to propozycja narzędzia, kasacja wyłącznie po decyzji człowieka. Pliki RÓŻNE (zgodne tylko w pierwszych 4 MB) NIE są kandydatami.", "",
