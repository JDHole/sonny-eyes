"""Logika CLI: raport ujecia v0. Wywolywane przez `python -m eyes measure`
(eyes/cli.py) i przez alias `scripts/measure.py` (dla starych komend).

measure --project "Nazwa" --files <plik1> <plik2> ...
        [--out X] [--config X] [--lut X] [--no-gpu] [--force]
        [--window-start 5 --window-len 20]
        [--cache-root X]  (przestarzale, alias --out - zgodnosc wsteczna)
"""
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    ap.add_argument("--out", default=None, help="jeden folder wyjsciowy (nadpisuje out_root/cache_root/reports_root)")
    ap.add_argument("--config", default=None, help="jawna sciezka do config.toml")
    ap.add_argument("--lut", default=None, help="wymus ten LUT na kazdy klip, niezaleznie od kamery")
...
def process_file(path: Path, *, args, cfg, hash_by_path: dict, file_paths: list[Path], lut_override: str | None) -> dict:
    ...
    id_ = keys_mod.compute_id(path, file_paths)  # file_paths = lista PO dedupie; brak root = wspolny korzen listy
    ...
    # LUT: --lut (CLI) wygrywa na kazdym klipie niezaleznie od kamery; inaczej
    # tabela [lut] configu po nazwie kamery malymi literami (klucz "fuji" itd.);
    # brak jednego i drugiego = pomiar bez LUT.
    lut_path = lut_override or (cfg.lut.get(kamera.lower()) if kamera else None)
    lut_dir = None
    lut_name = None
    lut_pomiarowy = None
    if lut_path:
        lut_dir, lut_name = decode_mod.ensure_lut_cached(lut_path, cache_root)
        lut_pomiarowy = Path(lut_path).name
    ...
    zrodlo = {
        "plik": path.name,
        # Bez wspolnego korzenia (vault/production_root) do skracania - pelna
        # sciezka absolutna, ukosniki znormalizowane do "/".
        "sciezka_rel": str(path.resolve()).replace("\\", "/"),
...
def main(argv=None, on_progress=None) -> int:
    ...
    for idx, path in enumerate(file_paths, 1):
        if path in dup_map:
            ...
            _log_i_postep({
                "id": keys_mod.compute_id(original, unique_paths),
                "plik": path.name,
            ...
            continue
        try:
            result = process_file(path, args=args, cfg=cfg, hash_by_path=hash_by_path, file_paths=unique_paths, lut_override=args.lut)
            _log_i_postep({
                "id": result["id"],
                "plik": result["plik"],
            ...
            _log_i_postep({
                "id": keys_mod.compute_id(path, unique_paths),
                "plik": path.name,
                "status": "blad",
                "czas_calkowity_s": None,
                "error": str(e),
